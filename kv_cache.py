...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_in, d_out, context_length, dropout, num_heads, qkv_bias=False):
        ...
        # regi buffer, cache_k, start none, not save to model
        self.register_buffer("cache_k", None, persistent=False)
        # regi buffer, cache_v, start none, not save to model
    ...
    def forward(self, x, use_cache=False):
        ...
        if use_cache:
            ...
            if self.cache_k is None:
                ...
                # keys_new = [batch, heads, 1_token,  head_dim]
        ...
        '''
            token 1:
            queries.shape: [1, 12, 1, 64]
            keys.shape = [1, 12, 1, 64]

            num_tokens_Q = 1
            num_tokens_K = 1
            mask_bool = self.mask.bool()[0:1, :1]  # [[True]]

            token 2:
            queries.shape: [1, 12, 1, 64]
            keys.shape = [1, 12, 2, 64]

            num_tokens_Q = 1
            num_tokens_K = 2
            mask_bool = self.mask.bool()[1:2, :2]  # [[True], [True]]
            ...
            num_tokens_Q = 1
            num_tokens_K = 3
            mask_bool = self.mask.bool()[2:3, :3]  # [[True], [True]]
        '''
        # num_tokens_Q always 1 token
        num_tokens_Q = queries.shape[-2]
        # num_tokens_K is not original big content, but num_tokens_Q building up num_tokens_K
        num_tokens_K = keys.shape[-2]

        if use_cache:
            mask_bool = self.mask.bool()[
                # 1. num_tokens_Q move 1 by 1
                # 2. num_tokens_K: building up by num_tokens_Q
                self.ptr_current_pos:self.ptr_current_pos + num_tokens_Q, :num_tokens_K
            ]

            # num_tokens_Q advance ptr_current_pos by 1
